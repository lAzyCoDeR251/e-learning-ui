const userApi = {
  ...
  getCourses: async (token, page, category, level, popularity) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/user/courses?page=${page}&category=${category}&level=${level}&popularity=${popularity}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  getCourseById: async (token, courseId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/user/courses/${courseId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  getUnAuthCourses: async (token) => {
    try {
      const response = await fetch(
        "http://localhost:5000/api/user/home-courses",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  enrollUserInCourse: async (token, courseId) => {
    ...
    try {
      await fetch("http://localhost:5000/api/course/enroll", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  getEnrolledCourses: async (token, userId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/course/enrolled-courses/${userId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
